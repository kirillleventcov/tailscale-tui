...
export function icmpPingCommand(ip: string, timeoutSec: number, platform: NodeJS.Platform = process.platform): string[] {
  if (platform === "win32") return ["ping", "-n", "1", "-w", String(timeoutSec * 1000), ip]
  // -n: no reverse DNS. Linux -W is seconds; macOS and the BSDs take milliseconds.
  if (platform === "linux") return ["ping", "-n", "-c", "1", "-W", String(timeoutSec), ip]
...
export async function icmpPing(ip: string, timeoutSec = 3): Promise<PingResult> {
  const via = `icmp ${ip}`
  const res = await run(icmpPingCommand(ip, timeoutSec), timeoutSec * 1000 + 1500)
  const rtt = parseIcmpPing(res.stdout)
  if (rtt !== null) return { rtt, via }
  if (res.timedOut || res.code <= 1) return { rtt: null, via }
  const text = (res.stderr.trim() || res.stdout.trim()).split("\n").pop() ?? `exit code ${res.code}`
  throw new BackendError(`ping ${ip}: ${text}`, "Check that the ping command works for your user")
}
