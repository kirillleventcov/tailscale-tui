...
export const MULLVAD_RELAYS_URL = "https://api.mullvad.net/www/relays/wireguard/"
...
export function parseRelayList(json: unknown): Map<string, string> {
  ...
  for (const r of json) {
    ...
    if (typeof hostname === "string" && hostname && typeof ip === "string" && ip) out.set(hostname.toLowerCase(), ip)
  }
  return out
}
...
export class MullvadRelays {
  ...
  async lookup(hostname: string): Promise<string> {
    ...
    if (!ip) throw new BackendError(`${hostname} is not in Mullvad's relay list`)
    return ip
  }
  ...
  private load(): Promise<Map<string, string>> {
    ...
    if (this.lastError && Date.now() - this.lastError.at < RETRY_AFTER_MS) return Promise.reject(this.lastError.error)
  ...
  private async fetch(): Promise<Map<string, string>> {
    try {
      const res = await fetch(this.url, { signal: AbortSignal.timeout(this.timeoutMs), headers: { accept: "application/json" } })
