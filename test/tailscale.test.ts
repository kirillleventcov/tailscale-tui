...
const statusFixture = {
  ...
  Health: ["systemd-resolved and NetworkManager are wired together incorrectly; MagicDNS will probably not work."],
  MagicDNSSuffix: "tail0000.ts.net",
  CurrentTailnet: { Name: "you@example.com", MagicDNSSuffix: "tail0000.ts.net", MagicDNSEnabled: true },
  Peer: {
    "nodekey:hel": {
      ID: "nHEL",
      HostName: "hel-vps",
    ...
    "nodekey:mullvad": {
      ...
      CurAddr: "",
      PeerRelay: "",
      Location: { Country: "USA", CountryCode: "US", City: "Los Angeles, CA", CityCode: "LAX", Latitude: 34.05, Longitude: -118.24, Priority: 100 },
...
describe("parsePong", () => {
  test("parses tailscale ping replies in ms, s and µs with the path", () => {
    expect(parsePong("pong from raspberrypi (100.89.133.58) via 192.168.0.141:41641 in 1ms\n")).toEqual({
      rtt: 1,
      via: "via 192.168.0.141:41641",
    })
    expect(parsePong("pong from x (100.64.0.1) via DERP(fra) in 1.5s")).toEqual({ rtt: 1500, via: "via DERP(fra)" })
    expect(parsePong("pong from x (100.64.0.1) via DERP(fra) in 800µs")?.rtt).toBe(0.8)
    expect(parsePong("pong from x (100.64.0.1) in 12ms")?.via).toBe("tailscale ping")
...
describe("icmp", () => {
  test("builds the platform's ping command", () => {
    expect(icmpPingCommand("203.0.113.1", 3, "linux")).toEqual(["ping", "-n", "-c", "1", "-W", "3", "203.0.113.1"])
    expect(icmpPingCommand("203.0.113.1", 3, "darwin")).toEqual(["ping", "-n", "-c", "1", "-W", "3000", "203.0.113.1"])
    expect(icmpPingCommand("203.0.113.1", 3, "win32")).toEqual(["ping", "-n", "1", "-w", "3000", "203.0.113.1"])
  })
  test("parses iputils, BSD and Windows replies", () => {
    expect(parseIcmpPing("PING 1.1.1.1 (1.1.1.1) 56(84) bytes of data.\n64 bytes from 1.1.1.1: icmp_seq=1 ttl=60 time=2.75 ms\n")).toBe(2.75)
    expect(parseIcmpPing("64 bytes from 1.1.1.1: icmp_seq=0 ttl=57 time=120.512 ms")).toBe(120.512)
    expect(parseIcmpPing("Reply from 1.1.1.1: bytes=32 time=12ms TTL=57")).toBe(12)
    expect(parseIcmpPing("Reply from 1.1.1.1: bytes=32 time<1ms TTL=57")).toBe(1)
    expect(parseIcmpPing("1 packets transmitted, 0 received, 100% packet loss, time 0ms")).toBeNull()
  })
})
...
describe("parseRelayList", () => {
  test("maps Mullvad relay host names to their public IPv4", () => {
    const map = parseRelayList([
      { hostname: "fi-hel-wg-203", ipv4_addr_in: "185.65.133.165", active: true, type: "wireguard" },
