...
import { fit, fmtLatency, humanBytes, parseTime, relTime, truncate } from "../src/format"
import { applyQuery, findNodeByName, matchesTokens, tokenize, type ExitNode, type Query } from "../src/model"
...
const nodes: ExitNode[] = [
  ...
  node({ id: "fra", city: "Frankfurt", country: "Germany", countryCode: "DE", isMullvad: true, tags: ["tag:mullvad-exit-node"], priority: 100 }),
  node({ id: "ber", city: "Berlin", country: "Germany", countryCode: "DE", isMullvad: true, online: false, priority: 90 }),
  node({ id: "nyc", city: "New York", country: "United States", countryCode: "US", active: true, os: "windows" }),
]
...
describe("format", () => {
  ...
  test("parseTime treats Go zero time as unset", () => {
    ...
    expect(parseTime("2026-01-02T03:04:05Z")?.toISOString()).toBe("2026-01-02T03:04:05.000Z")
...
describe("query", () => {
  ...
  test("filters", () => {
    expect(applyQuery(nodes, { ...base, filter: "mullvad" }, noLatency).map((n) => n.id).sort()).toEqual(["ber", "fra"])
    expect(applyQuery(nodes, { ...base, filter: "tailnet" }, noLatency).map((n) => n.id).sort()).toEqual(["hel", "nyc"])
    expect(applyQuery(nodes, { ...base, filter: "online" }, noLatency).map((n) => n.id)).not.toContain("ber")
  })
  test("search matches name, city, country code, os; ! excludes", () => {
    expect(applyQuery(nodes, { ...base, text: "german" }, noLatency).map((n) => n.id).sort()).toEqual(["ber", "fra"])
    expect(applyQuery(nodes, { ...base, text: "de !berlin" }, noLatency).map((n) => n.id)).toEqual(["fra"])
    expect(applyQuery(nodes, { ...base, text: "windows" }, noLatency).map((n) => n.id)).toEqual(["nyc"])
    expect(matchesTokens(nodes[0]!, tokenize("  FI   hel "))).toBe(true)
  })
  test("location sort groups by country then city, desc reverses", () => {
    const asc = applyQuery(nodes, { ...base, sort: "location" }, noLatency).map((n) => n.id)
    expect(asc).toEqual(["ber", "fra", "hel", "nyc"])
    const desc = applyQuery(nodes, { ...base, sort: "location", desc: true }, noLatency).map((n) => n.id)
    expect(desc).toEqual(["nyc", "hel", "fra", "ber"])
  })
  test("latency sort: measured first ascending, then pending, unmeasured, timeouts", () => {
    const lat = (id: string) => ({ hel: 8, fra: 30, ber: null, nyc: "pending" as const })[id]
    const out = applyQuery(nodes, { ...base, sort: "latency" }, lat).map((n) => n.id)
    expect(out).toEqual(["hel", "fra", "nyc", "ber"])
  })
  test("priority sort is descending by priority", () => {
    const out = applyQuery(nodes, { ...base, sort: "priority", filter: "mullvad" }, noLatency).map((n) => n.id)
