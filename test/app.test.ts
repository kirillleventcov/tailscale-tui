...
interface Harness {
  ...
  /** y of the first node row: header + toolbar + panel border + column header. */
  firstRow: number
  frame: () => string
  lines: () => string[]
...
/** Full layout: header(2) + toolbar(3) + panel border(1) + column header(1). */
const ROW_Y = 7
/** Every exit-node capable peer in the fixture (the non-exit "phone" peer is filtered out). */
const TOTAL = 15
const ONLINE = 12

async function boot(width = 120, height = 32, opts: FixtureOptions = {}): Promise<Harness> {
...
describe("tsexit UI", () => {
  test("renders status header, columns and nodes parsed from tailscale status", async () => {
    const h = await boot()
    const f = h.frame()
    expect(f).toContain("NO EXIT NODE")
    expect(f).toContain(`Exit nodes · ${TOTAL}`)
    expect(f).toContain(`${TOTAL} exit nodes (${ONLINE} online)`)
    ...
    expect(f).toContain("suggested ★ fi-hel-wg-203")
  ...
  test("Enter connects, d disconnects, and the CLI calls match", async () => {
    ...
    await h.setup.waitForFrame((f) => f.includes("CONNECTED via home-server"), { maxPasses: 40 })
    ...
    expect(h.backend.calls).toEqual(["set --exit-node=100.64.0.20", "set --exit-node="])
  })
  ...
  test("offline nodes need a second Enter", async () => {
    ...
    h.setup.mockInput.pressEnter()
    await h.setup.waitForFrame((f) => f.includes("CONNECTED via pixel-8"), { maxPasses: 40 })
  ...
  test("ping fills the latency column and shows how it was measured", async () => {
    ...
    await h.setup.waitForFrame((f) => /home-server.*\b12 ms/.test(f), { maxPasses: 40 })
    expect(h.frame()).toMatch(/Latency\s+12 ms\s+│[\s\S]*│\s+via 192\.168\.0\.20:41641/)
    ...
    h.setup.mockInput.pressEnter()
    h.setup.mockInput.pressKey("p")
    await h.setup.waitForFrame((f) => /fi-hel-wg-203.*6\.2 ms/.test(f), { maxPasses: 40 })
    expect(h.frame()).toMatch(/Latency\s+6\.2 ms\s+│[\s\S]*│\s+icmp 203\.0\.113\.203/)
  })

  test("P pings every visible online node and reports the result", async () => {
    const h = await boot()
    h.setup.mockInput.pressKey("p", { shift: true })
    await h.setup.waitForFrame((f) => f.includes(`Pinged ${ONLINE} nodes, ${ONLINE} replied`), { maxPasses: 80 })
  ...
  test("the Ping all chip pings the visible nodes with the mouse", async () => {
    ...
    const x = h.lines()[3]!.indexOf("Ping all")
    expect(x).toBeGreaterThan(0)
    await h.setup.mockMouse.click(x + 1, 3)
    await h.setup.waitForFrame((f) => f.includes("Pinging…"), { maxPasses: 40 })
  ...
  test("a toggles Tailscale's auto exit node; picking a node manually turns it off", async () => {
    ...
    await h.setup.waitForFrame((f) => f.includes("CONNECTED via fi-hel-wg-203 auto"), { maxPasses: 40 })
    expect(h.frame()).toContain("★ Auto on")
    expect(h.frame()).toContain("Tailscale now picks and follows the best exit node")
    ...
    h.setup.mockInput.pressKey("a")
    await h.setup.waitForFrame((f) => f.includes("★ Auto on"), { maxPasses: 40 })
    h.setup.mockInput.pressKey("a")
    await h.setup.waitForFrame((f) => f.includes("Auto exit node off, staying on fi-hel-wg-203"), { maxPasses: 40 })
    expect(h.backend.calls.at(-1)).toBe("set --exit-node=100.64.1.203")
    expect(h.frame()).toContain("CONNECTED via fi-hel-wg-203 100.64.1.203")
  })

  test("A connects to the suggested node once", async () => {
    const h = await boot()
    h.setup.mockInput.pressKey("a", { shift: true })
    await h.setup.waitForFrame((f) => f.includes("CONNECTED via fi-hel-wg-203"), { maxPasses: 40 })
    expect(h.frame()).toContain("★ Auto off")
  ...
  test("starts in auto mode when Tailscale already has it on", async () => {
    const h = await boot(120, 32, { autoExitNode: true, exitNodeId: "nFIHEL203CNTRL" })
    expect(h.frame()).toContain("CONNECTED via fi-hel-wg-203 auto")
    expect(h.frame()).toContain("★ Auto on")
    expect(h.selectedName()).toBe("fi-hel-wg-203")
...
describe("computeColumns", () => {
  test("drops optional columns from the right as the width shrinks", () => {
    expect(computeColumns(120).map((c) => c.id)).toEqual(["mark", "name", "status", "location", "latency", "os", "priority"])
    expect(computeColumns(120, false).map((c) => c.id)).toEqual(["mark", "name", "status", "location", "latency", "priority"])
    expect(computeColumns(40).map((c) => c.id)).toEqual(["mark", "name", "status"])
